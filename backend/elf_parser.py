...
if __name__ == "__main__":
    ...
    st_dies = zp._find_struct_die(args.struct_name)
    ...
    for CU_ in zp.dwarf.iter_CUs():
        for die_ in CU_.iter_DIEs():
            if die_.tag == "DW_TAG_variable":
                ...
                die_name = die_.attributes.get("DW_AT_name")
                # if die_name is not None:
                #     print(die_name.value.decode("utf-8"))
                if die_type_offset is not None and die_type_offset.value + die_.cu.cu_offset in [st_die.offset for st_die in st_dies]:
                    results.append(die_name.value.decode("utf-8"))

    if not results:
        print("\nNo variables of that struct type were found.")
    else:
        print("\n--- Summary ---")
        for var in set(results):
            print(f"Variable: {var:<30}")
